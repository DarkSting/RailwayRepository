...
class PerspectiveListView extends StatefulWidget {
  const PerspectiveListView({
    ...
    required this.onChangeFrontItem,
    this.backItemsShadowColor = Colors.transparent,
    this.enableBackItemsShadow = false,super.key
  });
...
class _PerspectiveListViewState extends State<PerspectiveListView> {
  ...
  void _pageListener() {
    ...
    double? val = _pageController!.page;

    if(val!=null){
      _pagePercent = (val- _currentIndex).abs();
    }
        ...
        children: [
          ...
          for (int index = 0; index < generatedItems; index++)
            (currentIndex > ((generatedItems - 2) - index))
              ...
              scale: lerpDouble(0.5, 1, (index + 1) / generatedItems)??0,
              translateY:
  ...
  Widget build(BuildContext context) {
    return Transform(
      ...
      transform: Matrix4.identity()!
        ...
        ..translate(0.0, lerpDouble(translateY, endTranslateY, factorChange)??0),
      child: Align(
        alignment: Alignment.topCenter,
        child: Container(
          height: heightItem,
